...
SET buy_method = INITCAP('&Type_de_paiement'),
   price = (SELECT SUM((tt.price)+
                           ((tt.price*NVL(w.class_pct, 0))/100)-
                           (((tt.price+((tt.price*NVL(w.class_pct, 0))/100))*NVL(CASE
          WHEN TO_CHAR(tt.departure_time, 'D') BETWEEN 1 AND 5 THEN discount_pct
          WHEN TO_CHAR(tt.departure_time, 'D') BETWEEN 6 AND 7 THEN discount_we_pct
          END, 0))/100))
